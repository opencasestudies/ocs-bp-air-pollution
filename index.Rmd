...
avocado response-what do you think of how I titled this section - if you still dont like it feel free to delete

hmmm avocado... the broom functions only work with this though to see the fitted and se values..
...
In this case, we actually also needed to write your model again! 
...
avocado update after deciding if you are removing the older method...even if you remove older method ... need to check if these now work with workflows and if they work in a simpler way.. like do we need pull?

After we fit our model, we can use the `broom` package to look at the output from the fitted model in an easy/tidy.   

There are three functions from `broom` that are especially helpful for this: 
...
So we can see that our performance isn't quite as good as what we had with the original model - especially if we look at the `rmse` value, which is much smaller for the original model.  
...
RF_wflow_fit %>% 
  ...
  vip(num_features = 10)
```
 avocado this was because I previously changed the state variable so I need to update this section about what the current diferences are
...
avocado Use this: https://juliasilge.com/blog/sf-trees-random-tuning/ as a guide...

We can tune for the number of each by using the `tune()` function in the `recipes` package called `step_ns()` where `ns` stands for natural splines. 
In order to tune for the number of knots or degrees of freedom, we can set the `deg_free` argument to `tune()`. 


This is helpful, because we aren't exactly sure what values of `mtry` and `min_n` achieve good accuracy yet keep our model generalizable for other data.
