...
To build (or train) our model, we use both the outcome and predictor variable(s).
...
Using this model, we can then predict the outcome from new observations with the same predictor variable(s) where have not observed the outcome. 
...
To build this machine learning model, our **outcome variable** is fine particulate mater (PM~2.5~) captured from air pollution monitors in the contiguous US from 2008. 
...
<details><summary> Click here to see a table about the set of predictor variables </summary>
...
We can do this using the `mutate_at()` function of the `dplyr` package and the `as.factor()` base function. 

AVOCADO: Should we use `across()` instead of `mutate_at()`? https://dplyr.tidyverse.org/reference/mutate_all.html 
...
By design, it provies summary statistics about variables in the dataset. 
...
First, we calculate the Pearson correlation coefficients between all predictor variables pairwise using the `cor()` function of the `stats` package (which is loaded automatically). Then we use the `corrplot::corrplot()` function. 
...
Becauses some variables in our data have extreme values, it might be good to take a log transformation. This can affect our estimates of correlation. 
```{r}
pm %>%
