...
However, our cross validet RMSE value of `r show_best(df_tuning, metric = "rmse", n =1)` is quite good because our range of true outcome values is much larger:`r range(test_pm$value)`.
...
```
We can see that our rmse is considerably larger with our test data, altough still not bad.

We could also use the `last_fit()` function of the `workflows` package. This function peforms 
