...
This package loads more modeling related packages like `rsample`, `recipes`, `parsnip`, `yardstick`,  and `dials`. 
We will also briefly cover the `workflows` and `tune` packages. 
...
avocado comment: there isnt a code book related to this data, Roger wrangled it for me, but the vaping case study actually uses code books for that data. Would you like to link to a generic code book and describe that process?
...
AVOCADO: when I go to compile the entire document, i get the following error. the files in the `/tmp` folder fo rme range from 0001 to 0100. but the figure below starts at 0101? 

```
File tmp/ds0101.png not found in resource path
...
- Demonstrate how to visualize geo-spatial data using `ggplot`
...
We used packages like `skimr`, `summarytools`, `corrplot`, `ggcorrplot`, and `GGally` to better understand our data. These packages gave can tell us how many missing values each variable has (if any), the class of each variable, the distribution of values for each variable, the sparsity of each variable, and the level of correlation between variables.  
...
AVOCADO: could we use a different link instead of to data camp? 
