...
It estimates parameters (estimating the required quantities and statistics required by the steps for the variables) for preprocessing and updates the model terms, as some of the predictors may be removed, this allows the recipe to be ready to use on other data sets. 
It **does not necessarily actually execute the preprocessing itself**, however we will specify in argument for it to do this so that we can take a look at the preprocessed data.
...
So far we have used `rsample` to split the data and `recipes` to assign variable and to specify and prep our pre-processing (as well as to optionally extract the preprocessed data).
