...
2) can easily modify preprocessing and parameter and hyper-parameter tuning making optimization easy
...
The`initial_split()` function allows us to specify how we want to split our data. Typically data is split into 3/4 for training and 1/4 for testing.This is the default proportion and does not need to be specified. However you can change the proportion using the `prop` argument, we will do that here for illustrative purposes. You can also specify a variable to stratify by with the `strata` argument. This is useful if you have imbalanced categorical variables and you would like to intentionally make sure that there are similar number of samples of the rarer categories in both the testing and training sets. Otherwise the split is performed randomly. 
...
If our dataset were large enough it might be nice then to stratify by state, but our data is not unfortunately not large enough. We will show how one would do this though for illustrative purposes. This option is more important for classification applications of machine learning than it is for prediction.
...
We are going to start with a linear regression but we will demonstrate how we can try different models.
...
We would like to use the ordinary least squares method to fit our linear regression. So we will tell `parnsip` that we want to use the `lm` package to implement our linear regression (there are many options actually). We will do so by using the `set_engine()` function of the `parsnip` package.
...
However a newer package called `workflows` allows us to keep track of both our preprocessing steps and our model specification. It also allows us to implement fancier optimizations in an automated way, so it is good to learn about it!
...
Ah, nice. Notice how it tells us both what our preprocessing steps are and our model specifications.

Now we can prepare the recipe (estimate the parameters) and fit the model to our training data all at once:
