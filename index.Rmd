...
If we were happy with our model performance with the training data, we could stop here and use the `yardstick` package to evaluate performance with our testing data. However, generally we would modify our model a bit until we were happy with the performance values that we achieved with our training data. This would likely involve tuning model hperparmaters which will talk about soon. However, let's pretend we were satisfied and wanted to look at our test data. 

First we would want to use the model to predict values for the monitors in the testing data.

Using `parsnip` we would need to use the baked data testing data. With the `workflows` package, we could use the raw testing data. Importantly, ID variables are not delt with as nicely as with the `workflows` package so we would need to remove them. We did this above when we used the `juiced_train` data as well.

```{r}

 baked_test_pm_ready <-baked_test_pm %>%select( -"id", -"fips")
 values_pred_parsnip <-predict(PM_fit, baked_test_pm_ready)
 values_pred_parsnip
...
yardstick::mae(values_pred_wfs, truth = value, estimate = .pred)
...
Another powerful method for improving your confidence in the etimates of performance is called cross validation.
...
# train_pm[["state"]]<- as.factor(train_pm[["state"]])
# train_pm[["county"]]<- as.factor(train_pm[["county"]])
# train_pm[["city"]]<- as.factor(train_pm[["city"]])
#train_2 <-train_pm %>% select(-fips, -id)
...
Fitting the data using cross validation:
```{r}
...
Looks like the model with the lowest rmse is the linear regression model with 5 degrees of freedom for latitude and longitude.
