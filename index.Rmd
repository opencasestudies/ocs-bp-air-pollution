...
This package loads more modeling related packages like `rsample`, `recipes`, `parsnip`, `yardstick`,  and `dials`. We will also briefly cover the `workflows` and `tune` packages. 
...
3. Ability to implement tidymodels packages such as `rsample` to split the data into traning and testing sets as well as cross validation sets.
...
Another reason we should look out for correlation is that we dont want to include redudant variables. This can add unnecessary noise to our algorithm causing a reduction in prediction accuracy and it can cause our algroithm to be unnecessarily slower. Finally, it can also make it difficult to interpret what variables are actually predictive.
...
We don't want to remove some of our variables, like the `CMAQ` and `aod` variables, we can specify this using the `-` sign before the names of these variabels like so:
...
Variables have low variance if all the values are very similar, the values are very sparse, or if they are highly imbalanced. Again we dont want to remove our `CMAQ` and `aod` variables.
...
Avocado... I dont think this got updated... at one point I did in california or not... so might want to delete this if it looks like later I didnt change anything about `state`
...
Notice, it looks like we gained the `log_prisec_length_25000` back with this recipe using the data with our changes to `state` and `city`. avocado... may need to remove state here...
...
<details><summary> Click here learn how this was done before the `workflows` package was developed </summary>
...
If yo have previously used `Tidymodels` but are new to `workflows` it may be useful to see what is different about fitting a model with each method.
...
3. `augment()` - shows the data, the fitted values (`.fitted`), the standard error (`.se.fit`) for each value and more 
...
Using the `parsnip` package, we can grab the predicted outcome values (or fitted values) like this: 
...
Alternatively, we can get the fitted values using the `augment()` function using the output from `parsnip`: 

```{r}
parsnip_fitted_values <- 
  augment(PM_fit[["fit"]], data = juiced_train) %>% 
  select(value, .fitted:.std.resid)

head(parsnip_fitted_values)
```

Alternatively, we can use the output from `workflows`:
...
Alternatively, we can get the fitted values using the `augment()` function using the output from `workflows`: 

```{r}
wf_fitted_values <- 
  augment(wf_fit[["fit"]], data = juiced_train) %>% 
  select(value, .fitted:.std.resid)

head(wf_fitted_values)
```

Either way, this gives us the same fitted values
...
AVOCADO: could we add the distance metrics themselves here? avocado comment(I added the formulas... is that what you meant?)
...
AVOCADO: why $k$ (or $v$) instead of just one? 

First, the training set is divided into $k$ (or $v$) equally sized smaller pieces. 

Next, the model is trained on the model on $k$-1 or $v$-1 subsets of the data iteratively (removing a different $v$ or $k$ until all possible $k$-1 or $v$-1 sets have been evaluated) to get a sense of the performance of the model. 
...
AVOCADO: maybe add a sentence explaining what we see? 
...
AVOCADO: not sure how to interpret the above output? or what it means? 
...
AVOCADO: what is `control_resamples()` doing? 
...
AVOCADO: maybe add a sentence here of what we see? 


### Model tuning

AVOCADO: maybe an intro sentence here what we mean by tuning? 

Now let's try some tuning.

Let's take a closer look at how the air pollution monitor values vary with the location (`loc_value`), specifically latitude (`lat`) and longitude (`lon`).
...
We can see that there does not appear to be a linear relationship for either of these predictors. 
Therefore, we might want to think about using  [splines](https://www.math.uh.edu/~jingqiu/math4364/spline.pdf){target="_blank"} or [natural cubic splines](https://towardsdatascience.com/numerical-interpolation-natural-cubic-spline-52c1157b98ac){target="_blank"} or just the [tune getting started guide ](https://tidymodels.github.io/tune/articles/getting_started.html){target="_blank"} to model the relationship in our training data more closely. 
For example, in the latitude plot (left) if we had 2 lines and one break-point called a knot around 40, with the first line having a positive slope and the second with a negative slope this would fit the data more similarly to the blue line shown in the figure.

AVOCADO: maybe we should explain a bit more by what we mean splines? I'm not sure just linking out to various resources would be enough here? 

AVOCADO: should we explain what are knots or degrees of freedom? 

We can tune for the number of knots by using a step function in the `recipes` package called `step_ns()` where `ns` stands for natural splines. 
In order to tune for the number of knots or degrees of freedom, we can set the `deg_free` argument to `tune()`. 

Let's update our `novel_rec` recipe to add this step. 
...
This is helpful, because we aren't exactly sure how closely we should be following the relationship with the value and our longitude and latitude data in our training data to achieve good accuracy yet keep our model generalizable for other data.

This is when our cross validation methods become really handy. 
For example, we can test out different values for the `deg_free` argument and see how our model performance varies across our training folds to try to find the optimal value.

Also, it is a good idea to do this for individual predictors, because you can name each with the `tune` argument so that you can keep track of it later. 
We can see what we intend to tune with the `parameters()` function of the `dials` package. 
...
AVOCADO: While I generally understand what's happening in the `dials::parameter()` function, i'm not sure I actually get it. Could we provide a bit more text here explaining what's happening? or what the output above (`pm_param`) is showing? 
...
AVOCADO: everything after this point until the start of the next section is really confusing to me. I see sometimes you have examples commented out, but i'm not sure we should include code if we aren't going to talk about it? Part of me wonders if this section should be in a "Click more here if.." down arrow? That or we need to talk about how to make this section more clear. For now 

Generally you could use the `grid_*()` functions of the `dials` package to create the different combinations of degrees of freedom to test for both variables to optimize the model. 
In our case we can visibly see that if we add more than say 4 or 5 degrees of freedom we will likely over-fit the data. 
So instead of using these functions we will create our own grid using the base `seq()` and `expand.grid()` functions.
...
AVOCADO: I'm not sure what these numbers mean? 
...
AVOCADO: I got this output after running the code chunk above `df_tuning`: 
```
> df_tuning
#  4-fold cross-validation 
cannot open compressed file '/Users/shicks/Documents/github/websites/opencasestudies/ocs-bp-air-pollution/.Rproj.user/shared/notebooks/4922A1-index/1/4E3CFC04439A89E6/cmq1o1r6gyahs_t/_rs_rdf_1d6f4f240c09.rdf', probable reason 'No such file or directory'Error in gzfile(file, "wb") : cannot open the connection
```
AVOCADO: Any idea what it means? I'm going to comment it out for now so i can compile

AVOCADO: Assuming my error can get fixed, could we write a sentence explaining what we see at output? 
...
AVOCADO: Could we write a sentence explaining what we see at output? also about what the functions `collect_metrics()` and `show_best()` are? 
...
In the next few sections, we will demonstrate a few other machine learning models. 

### Linear Regression Model with PCA

AVOCADO: I think we should take out this section (Linear Regression Model with PCA). My sense is we would need to do quite a bit more explanation of what is PCA and I'm hestitant to make this case study longer... happy to hear your thoughts on this though. For now I'm not going to edit it until we have a chance to talk. 
...
And we can compare this with our previous performance using the linear model:

AVOCADO: if we keep this section, could it be renamed something like `resample_lm_fit`? 
...
AVOCADO: did you mean `rsq` value? I think the `rmse` is smaller for the original linear model? 

### Random Forest

Now for one last recipe, we are going to predict our outcome variable (air pollution) using a decision tree method called [random forest](https://en.wikipedia.org/wiki/Random_forest){target="_blank"}.
...
AVOCADO: why the specific number 53 levels? 
...
AVOCADO: not sure what the "10 and 3" mean? 
...
Finally, we fit the data to the model using either just `parsnip` 
...
RF_wflow_fit %>% 
  ...
  vip(num_features = 10)
```
AVOCADO: i got a different output than you did with `state` being the top, followed by `CMAQ`, then `county_area`. I'm guessing we needed to set a seed somewhere? The text below might not be right? 
...
However, our cross validated mean rmse value of 1.79 is quite good because our range of true outcome values is much larger: (`r round(range(test_pm$value),3)`).

### Final model performance evaluation

Now that we have decided that we have reasonable performance with our training data, we could stop here and use the `yardstick` package (and `tune` if using `workflows` to fit our model) to evaluate performance with our testing data. 

Here, we will use the random forest model that we built to predict values for the monitors in the testing data.
...
AVOCADO: maybe remind readers what is baked testing data? 

Importantly, ID variables are not dealt with as nicely as with the `workflows` package so we would need to remove them. 
We did this above when created the processed training data for this model, the `juiced_train` data as well.

First, let's extract the predicted outcome values on the testing data. 
...
AVOCADO: I'm getting the following output: 

```
Error in predict.randomForest(object = object$fit, newdata = new_data) : missing values in newdata
```

AVOCADO: any ideas on what i should do? 

AVOCADO: Setting the rest of the code chunks to `eval=FALSE` for now

Alternatively, you can also do this with `workflows`: 
```{r,eval=FALSE}
values_pred_wfs <- 
...
Next we calculate the performance metrics with `yardstick`
```{r,eval=FALSE}
yardstick::metrics(values_pred_wfs, 
                   truth = value, estimate = .pred)
```
...
AVOCADO: maybe remind readers what each package does by copying and pasting text from when you originally introduced the packages? that or link to the section? 
...
AVOCADO: maybe add a sentence about what the `ne_countries()` function does? 

We will start with getting an outline of the US with the `ne_countries()` function of the `rnaturalearth` package.

AVOCADO: So I definitely have the `rnaturalearthdata` package installed, but for someone reason every time I go to run the code below it says that I need to install the package. Just curious -- what version of R are you using? Setting the code chunks to false for now. 
...
The `sf` package which is short for simple features creates a data frame about this graphical data so that we can work with it.
...
In this case, the output structure for the training data fit is slightly different using `randomForest`. 
The fitted values are called `predicted` and the `broom` functions like `tidy()` and `augment()` will not work. 
So we will manually grab the predicted training data values.
...
training_RF_estimates <- 
  tibble(".pred" = training_RF_estimates,
         value= train_pm$value, 
         fips = train_pm$fips, 
         county = train_pm$county, id =train_pm$id)
...
AVOCADO: I would suggest adding more text in between the plots above in this section explaining what we see. 
...
Our goal was to able to predict air pollution on monitors where we only had predictor variables (or features) without having observed a corresponding measurement of air pollution.
...
We would like to acknowledge [Roger Peng](http://www.biostat.jhsph.edu/~rpeng/), [Megan Latshaw](https://www.jhsph.edu/faculty/directory/profile/1708/megan-weil-latshaw), and [Kristen Koehler](https://www.jhsph.edu/faculty/directory/profile/2928/kirsten-koehler)for assisting in framing the major direction of the case study.
