...
The model needs to first be fit to the training data. First the method in which the model will be fit is specified (regression, decision tree etc.).  Then in both classification and prediction, the model is fit to the traing data and uses the explanatory variables to estimate numeric values (in the case of prediction) or categorical values (in the case of classification) of the outcome variable of interest. If the model fits well than these estimated values will be very similar to the true outcome variable values. If the model does not fit well, than these estimates will be more dissimal from the true outcome variable. In this case, aspects about the model may need to be modified to improve the similarity of the estimates with that ofthe true outcome values. One way to optimize model performance is a process called tuning in which model [parameters and hyperparameters](https://www.datacamp.com/community/tutorials/parameter-optimization-machine-learning-models){target="_blank"} are tested to determine the best option for model performance.
...
NOTE this next section could be removed if it is confusing... and we would therefore update the orginal simple_rec recipe. But it shows what I had difficulty with - the new levels in testing data..
