...
```{r, eval=FALSE}
OCSdata::raw_data("ocs-bp-air-pollution")
```
...
```{r, eval=FALSE}
OCSdata::imported_data("ocs-bp-air-pollution")
```
...
Then, you may download the wrangled data `.rda` file using the following function:

```{r, eval=FALSE}
OCSdata::wrangled_rda("ocs-bp-air-pollution")
```
...
However, we can use the `tidy` function if we first use the `extract_fit_parsnip()` function.
...
If you were starting the case study around here, you can download the data (`.rda`) using the `OCSdata` package:

```{r, eval=FALSE}
# install.packages("OCSdata")
# library(OCSdata)
wrangled_rda("ocs-bp-air-pollution")
```
...
<details> <summary> Click here for a step-by-step guide on how to download and load the data. </summary>
